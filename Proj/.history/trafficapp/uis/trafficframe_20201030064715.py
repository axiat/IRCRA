...
class WTrafficDialog(QDialog):
    ...
    def init_dev(self):
        ...
        self.signal_brightness.connect(self.th_1.recv_brightness)
        self.signal_brightness.connect(self.th_2.recv_brightness)
        self.signal_brightness.connect(self.th_3.recv_brightness)
        self.signal_brightness.connect(self.th_4.recv_brightness)
        self.signal_brightness.connect(self.th_5.recv_brightness)
        self.signal_brightness.connect(self.th_6.recv_brightness)
        self.signal_brightness.connect(self.th_7.recv_brightness)
        # 绑定视频槽函数
    ...
    def show_video(self, data, h, w, c, th_id, dataT):
        qimg = QImage(data, w, h, w*c, QImage.Format_RGB888)
        qpixmap = QPixmap.fromImage(qimg)
        # img = QImageToCvMat(qimg)
        # img1 = timeImg(img)
        # # img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2RGB)
        # timeQimg = QImage(img1.tobytes(), w, h, w*c, QImage.Format_RGB888)
        timeQimg = QImage(dataT, w, h, w*c, QImage.Format_RGB888)
        timeQpixmap = QPixmap.fromImage(timeQimg)
            ...
            if self.main_id == 1:
                ...
                if self.is_sub_dialog:
                    self.show_video_in_label(
                        self.main_video_dlg.ui.lbl_video, timeQpixmap)
    ...
    def modify_bright(self, val):
        print("调整亮度值：", val)
        self.signal_brightness.emit(val)
